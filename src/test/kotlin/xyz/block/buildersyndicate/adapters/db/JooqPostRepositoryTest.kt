...
class JooqPostRepositoryTest {
    ...
    fun `update post then findById reflects changes`() {
        ...
        val created = postRepository.create(post)
        postRepository.update(created.copy(
            title = "Updated Title",
    ...
    fun `listRecent returns posts ordered newest-first`() {
        val post1 = postRepository.create(Post(
            authorId = testUser.id!!,
            title = "First Post",
            body = "Created first"
        ))
        Thread.sleep(1100)
        val post2 = postRepository.create(Post(
            authorId = testUser.id!!,
            title = "Second Post",
            body = "Created second"
        ))
        Thread.sleep(1100)
        val post3 = postRepository.create(Post(
            authorId = testUser.id!!,
            title = "Third Post",
            body = "Created third"
        ))
    ...
    fun `listByAuthor returns only posts by that author`() {
        val otherUser = userRepository.create(User(
